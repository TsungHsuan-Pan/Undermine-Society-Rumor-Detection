...
class InputFeatures(object):
    ...
    def __init__(self, label, input_ids, attention_mask, token_type_ids=None):
...
def convert_examples_to_features(
    ...
    for i in range(len(examples)):
        ...
        feature = InputFeatures(label=labels[i], **inputs)
...
class GoEmotionsProcessor(object):
    ...
    def get_labels(self):
        labels = []
        label_dir = ''
        if self.args.last_layer_mode == "power_set":
            label_dir = 'data/' + self.args.model_name_or_path.split('/')[1]
        else:
            label_dir = self.args.data_dir
        with open(os.path.join(label_dir, self.args.label_file), "r", encoding="utf-8") as f:
            for line in f:
...
def load_and_cache_examples(args, tokenizer, mode):
    ...
    if "xlm-roberta" not in args.model_name_or_path:
